...
function(require, when, timeout, array, object, createModuleLoader, Lifecycle, Resolver, basePlugin) {
	...
	function createScope(scopeDef, parent, options) {
		 ...
		 * that scope.
		 *
		 * @param ns {String} name or namespace to match components and scopes' `_origin` fields.
		 ...
		 *                    error occurs.
		 */
		function destroyChildren(ns) {
			var path = ns.split('.');
			var container = components;
			var i, j;

			// First use the first N-1 segments of path[] to descend into the relevant scope
			for (i = 0; i < path.length-1; i++) {
				container = container[path[i]];
				// It's an error if container is not a scope ('s components object)
				if (typeof container !== 'object' ||
						typeof container.wire !== 'function' ||
						container.wire.name !== 'wireChild') {
					return when.reject('Invalid name: ' + ns);
				}
			}

			// Now match the last segment of path against the `_origin` of components and
			...
			function destroyComponent(proxy) {
				return when(lifecycle.shutdown(proxy), function() {
					...
					if (container[proxy._name] === proxy.target) {
						delete container[proxy._name];
					}
			...
			var proxyArray = container._scope._proxiedComponents;
			j = proxyArray.length - 1;
			for (i = 0; i <= j; i++) {
				if (proxyArray[i]._origin === segment) {
			...
			var scopeArray = container._scope._childScopes;
			j = scopeArray.length - 1;
			for (i = 0; i <= j; i++) {
				if (scopeArray[i].destroy) {
					if (scopeArray[i]._origin === segment) {
